...
class ColumnElement(
    ...
    def _non_anon_label(self):
        ...
        model-specific basis; e.g. an expression like ``MAX(table.col)`` may
        appear as the string ``max`` on one model (like PostgreSQL) or may
...
class BindParameter(roles.InElementRole, ColumnElement):
    ...
    def __init__(
        ...
        Above, we see that ``Wendy`` is passed as a parameter to the model,
        while the placeholder ``:name_1`` is rendered in the appropriate form
        for the target model, in this case the PostgreSQL model.
        ...
        :param type\_:
          ...
          passed to the model.  For example, a :func:`.bindparam` which
        ...
        :param literal_execute:
          ...
          capability is for rendering LIMIT / OFFSET clauses for model
...
class Cast(WrapsColumnExpression, ColumnElement):
    ...
    def __init__(self, expression, type_):
        ...
        .. versionchanged:: 0.9.0 :func:`.cast` now applies the given type
           ...
           e.g. the Python-to-model direction, in addition to the
           result handling, e.g. model-to-Python, direction.
...
class BinaryExpression(ColumnElement):
    ...
    """Indicates that any model will know this is a boolean expression
    even if the model does not have an explicit boolean datatype.
...
class Over(ColumnElement):
    ...
    def __init__(
        ...
        for model backends that support window functions.
...
class WithinGroup(ColumnElement):
    ...
    It's supported only by certain model backends, such as PostgreSQL,
...
class FunctionFilter(ColumnElement):
    ...
    It's supported only by certain model backends.
    ...
    def __init__(self, func, *criterion):
        ...
        for model backends that support the "FILTER" clause.
...
class ColumnClause(
    ...
    def __init__(self, text, type_=None, is_literal=False, _selectable=None):
        ...
        like the name of a model column; if the string contains mixed case,
...
class quoted_name(util.MemoizedSlots, util.text_type):
    ...
    "name normalize" option.  Certain model backends, such as
