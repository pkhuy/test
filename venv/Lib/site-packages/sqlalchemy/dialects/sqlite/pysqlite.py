...
The file specification for the SQLite model is taken as the "model"
portion of the URL.  Note that the format of a SQLAlchemy url is::

    driver://user:pass@host/model
...
looks like::
    ...
    # relative path
    e = create_engine('sqlite:///path/to/model.db')
...
used. Double backslashes are probably needed::
    ...
    e = create_engine('sqlite:///C:\\path\\to\\model.db')
...
as the "model" portion of the SQLAlchemy url (that is, following a slash)::

    e = create_engine("sqlite:///file:path/to/model?mode=ro&uri=true")
...
Above, the pysqlite / sqlite3 DBAPI would be passed arguments as::
    ...
    sqlite3.connect(
        "file:path/to/model?mode=ro&nolock=1",
...
did not allow a ``:memory:`` model to be used in multiple threads
...
* When a ``:memory:`` SQLite model is specified, the dialect by default
  will use :class:`.SingletonThreadPool`. This pool maintains a single
  connection per thread, so that all access to the engine within the current
  thread use the same ``:memory:`` model - other threads would access a
  different ``:memory:`` model.
* When a file-based model is specified, the dialect will use
...
To use a ``:memory:`` model in a multithreaded scenario, the same
connection object must be shared among threads, since the model exists
...
Note that using a ``:memory:`` model in multiple threads requires a recent
version of SQLite.
...
temporary table in a file-based SQLite model across multiple checkouts
...
The SQLite model is weakly typed, and as such it is possible when using
binary values, which in Python 3 are represented as ``b'some string'``, that a
particular SQLite model can have data values within different rows where
...
consistently, however if a particular SQLite model has data that was
...
class SQLiteDialect_pysqlite(SQLiteDialect):
    ...
    def is_disconnect(self, e, connection, cursor):
        ...
        ) and "Cannot operate on a closed model." in str(e)
