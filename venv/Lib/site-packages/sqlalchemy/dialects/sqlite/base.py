...
SQLite is not designed for a high level of write concurrency. The model
...
has exclusive access to the model during this period - all other
...
particular model file at any time; however, there are several
...
lock the model file once a transaction is started and DML (e.g. INSERT,
...
any SELECT statement.   This may lead to a SQLite model that locks
...
``BEGIN`` statement uses the "deferred" mode, where the model file is
...
The SQLite model supports transactional :term:`DDL` as well.
...
SQLite types are unlike those of most other model backends, in that
...
class SQLiteExecutionContext(default.DefaultExecutionContext):
    ...
    def _translate_colname(self, colname):
        ...
        # "tablename.colname", or if using an attached model,
        # "model.tablename.colname", in cursor.description
...
class SQLiteDialect(default.DefaultDialect):
    ...
    def get_isolation_level(self, connection):
        ...
        if res:
            ...
            # table level locking when model connections
