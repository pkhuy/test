...
in use as well as the network address of the model.   Assuming a datasource
...
model name to connect towards may be specified locally in the URL, rather
...
Some model servers are set up to only accept access tokens for login. For
...
model using Azure credentials::
    ...
    TOKEN_URL = "https://database.windows.net/"  # The token URL for any Azure SQL model

    connection_string = "mssql+pyodbc://@my-server.model.windows.net/myDb?driver=ODBC+Driver+17+for+SQL+Server"
...
will continue to maintain active model connections**, even when the
