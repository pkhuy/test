...
.. note::
    ...
    SQL Server model will instead reject the ``CREATE TABLE`` statement.
...
other words the model's configured default is used. This will
render::
...
* The flag begins with the value ``None``, before a model connection is
...
qualifier, that is, including the model name and owner name as separate
...
"model" and "owner"  components in order to correctly query the SQL
...
To control how the schema name is broken into model / owner,
...
To individually specify both model and owner name with special characters
...
.. versionchanged:: 1.2 the SQL Server dialect now treats brackets as
   identifier delimiters splitting the schema into separate model
...
model level. This allows, for instance, to run a model that
is compatible with SQL2000 while running on a SQL2005 model
server. ``server_version_info`` will always return the model
server version information (in this case SQL2005) and not the
compatibility level information. Because of this, if running under
a backwards compatibility mode SQLAlchemy may attempt to use T-SQL
statements that are unable to be parsed by the model server.
...
Enabling snapshot isolation for the model as a whole is recommended
...
class ROWVERSION(TIMESTAMP):
    ...
    model as itself; the returned datatype will be
