...
To connect to a MariaDB model, no changes to the model URL are required::
...
backing model reports as MariaDB.  Based on this flag, the dialect
...
and is not compatible with a MySQL model.    To use this mode of operation,
...
detection detects that the backing model is not MariaDB.
...
"mariadb" prefix will be present in the option names when the model URL
...
.. seealso::
    ...
    with timed out connections as well as model restarts.
...
that is created in a MySQL / MariaDB model with a storage engine
...
foreign key declarations are *always* received from the model
...
model itself, especially if model reflection features are
to be used.
...
the model connection will return true for the value of
``SELECT @@autocommit;``.
...
model that includes codepoints more than three bytes in size, this new
charset is preferred, if supported by both the model as well as the client
DBAPI, as in::
...
emit a warning when attempting to pass binary data to the model, while a
...
existing model structures.  The detection is entirely automatic and
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _get_server_version_info(self, connection):
        # get model server version info explicitly over the wire
