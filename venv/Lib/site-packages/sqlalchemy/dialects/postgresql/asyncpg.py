...
.. warning::  The ``asyncpg`` model driver necessarily uses caches for
   ...
   to the PostgreSQL model which modifies the tables or other objects
   ...
   model engine.     If DDL changes are made from other model engines
   ...
   failed for type <oid>")`` if it refers to pooled model connections which
