...
* ``isolation_level``: This option, available for all PostgreSQL dialects,
  ...
  connection that is set immediately upon connection to the model before
...
PostgreSQL model that is open for "trust" connections.  This behavior can be
...
* For PostgreSQL 9.1 and above, the ``client_encoding`` parameter may be
  passed in the model URL; this parameter is consumed by the underlying
...
* For all PostgreSQL versions, psycopg2 supports a client-side encoding
  value that will be passed to model connections when they are first
  ...
  .. tip:: The above ``client_encoding`` parameter admittedly is very similar
      ...
      passed to the model connection using ``SET client_encoding TO
...
* A common way to set up client encoding with PostgreSQL databases is to
  ...
  consistently of one encoding in all databases::
    ...
    # client_encoding = sql_ascii # actually, defaults to model
                                 # encoding
    client_encoding = utf8
...
it is detected that the target model has the HSTORE type set up for use.
