...
examples which assume the usage of an autoincrement-capable model.   To
...
The Oracle model supports "READ COMMITTED" and "SERIALIZABLE" modes of
...
.. note:: The implementation for the
   ...
   its first connection to the model in order to acquire the
   ...
   model-related failure, the level is assumed to be "READ COMMITTED".  No
...
model migration that wishes to "DROP CONSTRAINT" on a name that was
...
of the generation of truncated names, and to fully review and test all model
...
The Oracle model supports a limited form of RETURNING, in order to retrieve
...
present as a "marker" for this type; additionally, when a model column
...
.. versionchanged:: 0.9.4 Added :class:`_oracle.DATE` to subclass
   ...
   for migrating schemas to other model backends.
...
check for such limitations, only the model will.
