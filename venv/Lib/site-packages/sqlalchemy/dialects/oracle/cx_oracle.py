...
cx_Oracle provides several methods of indicating the target model.  The
...
Alternatively, if no port, model name, or ``service_name`` is provided, the
...
The cx_Oracle dialect connects to this model service when ``myalias`` is the
hostname portion of the URL, without specifying a port, model name or
``service_name``::
...
"model name" portion of the URL as below::
...
the Oracle model is configured with a Unicode-aware character set, as well
...
In the case that the Oracle model is not configured with a Unicode character
...
For the unusual case that data in the Oracle model is present with a broken
...
class OracleDialect_cx_oracle(OracleDialect):
    ...
    def create_connect_args(self, url):
        ...
        if database or service_name:
            # if we have a model, then we have a remote host
            port = url.port
            if port:
                port = int(port)
            else:
            ...
            if database and service_name:
                raise exc.InvalidRequestError(
                    ...
                    'be used with a "model" part of the url'
                )
            if database:
