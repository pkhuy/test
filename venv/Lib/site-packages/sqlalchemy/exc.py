...
class NoSuchModuleError(ArgumentError):
    """Raised when a dynamically-loaded module (usually a model dialect)
...
class InvalidatePoolError(DisconnectionError):
    ...
    entire pool should be invalidated, as the model has been restarted.
...
class NoResultFound(InvalidRequestError):
    """A model result was required but none was found.
...
class MultipleResultsFound(InvalidRequestError):
    """A single model result was required but more than one were found.
...
class UnboundExecutionError(InvalidRequestError):
    """SQL was attempted without a model connection to execute it on."""
...
class DBAPIError(StatementError):
    """Raised when the execution of a model operation fails.

    Wraps exceptions raised by the DB-API underlying the
    model operation.  Driver-specific implementations of the standard
