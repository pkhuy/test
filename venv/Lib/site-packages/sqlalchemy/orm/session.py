...
class Session(_SessionClassMethods):
    ...
    def __init__(
        ...
        :param autoflush: When ``True``, all query operations will issue a
           ...
           not be called repeatedly in order for model queries to retrieve
        ...
        :param expire_on_commit:  Defaults to ``True``. When ``True``, all
           ...
           transaction will load from the most recent model state.
        ...
        :param twophase:  When ``True``, all transactions will be started as
            ...
            of the model in use along with an XID.  During a
            :meth:`~.commit`, after :meth:`~.flush` has been issued for all
            attached databases, the :meth:`~.TwoPhaseTransaction.prepare`
            method on each model's :class:`.TwoPhaseTransaction` will be
            called. This allows each model to roll back the entire
    ...
    def begin_nested(self):
        ...
        The target model(s) and associated drivers must support SQL
    ...
    def rollback(self):
        ...
        model transaction if no nested transactions are in effect, or
        ...
        the topmost model transaction, discarding any nested
    ...
    def commit(self):
        ...
        the outermost model transaction.

        If :term:`2.0-style` use is in effect via the
        :paramref:`_orm.Session.future` flag, the outermost model
        ...
        rather than the actual model transaction, if a transaction
        is in progress.
    ...
    def close(self):
        ...
        .. tip::
            ...
            the :class:`_orm.Session` will release all model connections
            and ORM objects.

        .. versionchanged:: 1.4  The :meth:`.Session.close` method does not
           immediately create a new :class:`.SessionTransaction` object;
           instead, the new :class:`.SessionTransaction` is created only if
           the :class:`.Session` is used again for a model operation.
    ...
    def invalidate(self):
        ...
        This can be called when the model is known to be in a state where
    ...
    def no_autoflush(self):
        ...
        of objects which involve existing model queries,
    ...
    def refresh(self, instance, attribute_names=None, with_for_update=None):
        ...
        the model to refresh column-oriented attributes with the current
        ...
        in model state outside of that transaction.   Refreshing
        attributes usually only makes sense at the start of a transaction
        where model rows have not yet been accessed.
    ...
    def expire_all(self):
        ...
        in model state outside of that transaction.
            ...
            if obj is not None:
                ...
                if (
                    ...
                    raise exc.FlushError(
                        ...
                        "auto-generated value, check that the model table "
    ...
    def add(self, instance, _warn=True):
        ...
        Its state will be persisted to the model on the next flush
        operation.
    ...
    def delete(self, instance):
        ...
        The model delete operation occurs upon ``flush()``.
    ...
    def merge(self, instance, load=True, options=None):
        ...
        to load the object from the model based on primary key, and if
        ...
        :param load: Boolean, when False, :meth:`.merge` switches into
         ...
         events as well as all model access.  This flag is used for
        ...
        :param options: optional sequence of loader options which will be
         ...
         operation loads the existing version of the object from the model.
    ...
    def enable_relationship_loading(self, obj):
        ...
        will attempt to load a value from the model using this
        ...
        **not** participate in any persistence operations; its state
        ...
        If the object instead represents an existing identity in the model,
    ...
    def flush(self, objects=None):
        """Flush all the object changes to the model.

        Writes out all pending object creations, deletions and modifications
        to the model as INSERTs, DELETEs, UPDATEs, etc.  Operations are
        ...
        changes from Python to the model's transaction buffer.
        ...
        try:
            ...
            if not objects and self.identity_map._modified:
                ...
                util.warn(
                    ...
                    "reset, and will not result in model updates. "
    ...
    def is_modified(self, instance, include_collections=True):
        ...
        * Instances present in the :attr:`.Session.dirty` collection may
          ...
          is the same as that loaded from the model, resulting in no net
          change here.
        * Scalar attributes may not have recorded the previously set
          value when a new value was applied, if the attribute was not loaded,
          or was expired, at the time the new value was received - in these
          cases, the attribute is assumed to have a change, even if there is
          ultimately no net change against its model value. SQLAlchemy in
...
class sessionmaker(_SessionClassMethods):
    ...
    takes place even if there are model errors::
...
def make_transient(instance):
    ...
    .. warning::
        ...
        * were not present in the query which loaded this object, such as that
        ...
        As the object is transient and un-associated with any model
