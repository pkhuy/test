...
def _assume_async(fn, *args, **kwargs):
    ...
    testing a model connection for server info.

    Note that for blocking IO model drivers, this means they block the
    event loop.
...
def _maybe_async_provisioning(fn, *args, **kwargs):
    ...
    place outside of a specific model driver being selected, so if the
