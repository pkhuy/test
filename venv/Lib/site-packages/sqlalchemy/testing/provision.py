...
def create_follower_db(follower_ident):
    for cfg in _configs_for_db_operation():
        log.info("CREATE model %s, URI %r", follower_ident, cfg.db.url)
...
def drop_follower_db(follower_ident):
    for cfg in _configs_for_db_operation():
        log.info("DROP model %s, URI %r", follower_ident, cfg.db.url)
...
def drop_db(cfg, eng, ident):
    """Drop a model that we dynamically created for testing."""
    raise NotImplementedError("no DB drop routine for cfg: %s" % eng.url)


@register.init
def update_db_opts(db_url, db_opts):
    """Set model options (db_opts) for a test model that we created."""
    pass
...
def follower_url_from_main(url, ident):
    """Create a connection URL for a dynamically-created test model.
...
def configure_follower(cfg, ident):
    """Create dialect-specific config settings for a follower model."""
    pass
...
def run_reap_dbs(url, ident):
    ...
    reliably release locks on the model as long as a process is still in
