...
which automatically generates mapped classes and relationships from a model
...
model from an existing model on the fly.  By addressing the issue strictly
...
The simplest usage is to reflect an existing model into a new model.
...
explicit table declaration::
    ...
    # we can reflect it ourselves from a model, using options
...
the information reflected about a model column before the :class:`_schema.Column`
...
class AutomapBase(object):
    ...
    def prepare(
        ...
        :param schema: When present in conjunction with the
         ...
         by the model connection is used.
